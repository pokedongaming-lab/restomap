import { InvalidWeightsError, } from './types';
const ALL_FACTORS = [
    'population',
    'traffic',
    'income',
...
// ─── Engine ─────────────────────────────────────────────────────────────────
export class LocationScoringEngineImpl {
    ...
    _simulatedValues = null;
    _simulatedMissing = [];
    simulateFactorValues(values) {
        this._simulatedValues = values;
    }
    clearSimulatedValues() {
        this._simulatedValues = null;
    }
    simulateMissingFactors(factors) {
    ...
    async calculate(input) {
        ...
        const redistributedWeights = this.redistributeWeights(input.weights, activeFactors);
        // Weighted sum
        const breakdown = {};
        let total = 0;
    ...
    validateWeights(weights) {
        ...
        if (Math.abs(sum - 100) > WEIGHT_SUM_TOLERANCE) {
            throw new InvalidWeightsError(`Weights must sum to 100 (got ${sum.toFixed(2)})`);
        }
    }
    async fetchFactorScores(input, missingFactors) {
    ...
    redistributeWeights(weights, activeFactors) {
    ...
    deriveConfidence(missingFactors) {
...
//# sourceMappingURL=LocationScoringEngine.js.map